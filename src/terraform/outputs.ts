...
import { TerraformOutput } from "../types/index.js";
...
async function ensureInitialized(onInit?: () => void): Promise<void> {
  ...
  if (!fs.existsSync(terraformDir)) {
    ...
    await exec(
      "terraform",
      ["init", "-reconfigure", `-backend-config=path=${TFSTATE_PATH}`],
      {
        cwd: IAC_DIR,
      },
    );
...
export async function getTerraformOutputs(
  ...
  try {
    ...
    throw new Error(
      `Failed to get terraform outputs: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
