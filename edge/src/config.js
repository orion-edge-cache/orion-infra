 ...
 */
export function findMatchingRule(typeName) {
 ...
 * - '*' matches everything
 */
function matchesPattern(pattern, typeName) {
    // Exact match
    if (pattern === typeName)
        return true;
    // Wildcard match
 ...
 */
export function getCacheHeaders(entityTypes, isMutation) {
 ...
 */
export function getInvalidationTargets(mutationName) {
    const config = loadCacheConfig();
