...
import { extractEntities, extractEntityTypes, toSurrogateKey, isMutation, } from "./entities.js";
import { injectTypename, getOperationType, getOperationName, } from "./transform.js";
...
async function handleRequest(event) {
    ...
    const configDomain = config.get("compute_backend_domain") || "{{backend_domain}}";
    const configProtocol = config.get("compute_backend_protocol") || "{{backend_protocol}}";
    ...
    // Log request
    kinesisLogger.log(JSON.stringify({
    ...
    try {
        ...
        if (authSecret) {
            const authHeader = config.get("ORIGIN_AUTH_HEADER_NAME") || "Authorization";
...
// UTILITIES
// =============================================================================
function parseGraphQLHeader(raw) {
